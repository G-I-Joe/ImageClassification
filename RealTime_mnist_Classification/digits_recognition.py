...
while True:
        ...
        if len(points) != 0:
            ...
            if len(blackboard_cnts) >= 1:
                ...
                if cv2.contourArea(cnt) > 1000:
                    ...
                    newImage = cv2.resize(digit, (26, 26))
                    newImage = np.array(newImage)
                    newImage = newImage.astype('float32')/255
                    newer = np.tile(newImage ,3)
                    img = Image(pil2tensor(newer.reshape(26, 26, 3), np.float32))
                    print(img)
                    img.show()
    ...
    cv2.putText(frame, "Multilayer Perceptron : " + str(ans), (10, 410), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255, 255, 255), 2)
